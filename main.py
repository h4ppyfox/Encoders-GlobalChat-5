...
async def UpdateMemberCount():
    while True:
        ...
        await bot.change_presence(activity=discord.Game(f'🌎 | auf {len(bot.guilds)} Servern | 👤 | mit {usercount} Usern'))
...
async def addGlobal(ctx):
    if ctx.author.guild_permissions.administrator:
        if not guild_exists(ctx.guild.id):
            ...
            with open('servers.json', 'w') as f:
                json.dump(servers, f, indent=4)
            embed = discord.Embed(title="**Willkommen im GlobalChat von h4ppyfox?!?!?!**",
...
**<a:welt:831123015517208597> | Global Chat
...
async def sendAll(message: Message):
  ...
  icon = None  # set a default value or handle the absence of the attribute accordingly
  # rest of the code
  embed = discord.Embed(description=content, timestamp=datetime.now().astimezone(tz=de), color=0x2ecc71)
  embed.set_author(name=f'{author.name}#{author.discriminator}', icon=author.avatar_url)
  embed.set_footer(text=f'Gesendet von Server {message.guild.name}', icon)
icon = message.guild.icon_url
if icon:
    icon_url = icon
embed.set_thumbnail(url=icon_url)
embed.set_footer(text=f'{message.guild.name} | Server User: {message.guild.member_count}',
                 icon_url=f'{message.guild.icon_url}')
embed.set_thumbnail(url=message.author.avatar_url)
embed.add_field(name="** **", value="`📌`[Support](https://discord.gg/3KrjQnUza3)・`🤖`[Bot-Invite](https://discord.com/api/oauth2/authorize?client_id=1198211988753481818&permissions=8&scope=bot)", inline=False)
...
async def sendAllWillkommen(ctx):
    ...
    embed.add_field(
        name=f'Support & Bot⠀',
        value=
        f'[Support](https://discord.gg/3KrjQnUza3)・[Bot invite](https://discord.com/api/oauth2/authorize?client_id=1198211988753481818&permissions=8&scope=bot)',
        inline=False)
...
async def invite(ctx):
    embed=discord.Embed(title="\nInvite<a:Blob_Join:858565965854670878>\n", url="https://discord.com/api/oauth2/authorize?client_id=1198211988753481818&permissions=8&scope=bot",color=0x662a85)
    await ctx.send(embed=embed)
...
async def support(ctx):
		...
		embed = discord.Embed(
				...
				description = '**[Support server](https://discord.gg/3KrjQnUza3)**',
				color=0x662a85)
		await ctx.send(embed=embed)


bot.run('MTE5ODIxMTk4ODc1MzQ4MTgxOA.GGOOVk.fEwj6A2b5ygtZE3eAJoQcQQ0x6ZnW4MRviQWqk')